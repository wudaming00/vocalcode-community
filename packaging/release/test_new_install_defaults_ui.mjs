...
const dataNotes=[
  'Uninstalling VocalCode does not remove downloaded models, settings or kept History; Remove… does.',
  'Dragging VocalCode to the Bin does not remove these or kept History; Remove… does.'
];
...
test('first run offers the Back button as Enter, unticked on a new install',()=>{
  const step=between('<div class="fr-step" id="frKeyStep"','<div class="fr-step" id="frTryStep"');
  const box=step.match(/<input type="checkbox" id="frBackEnter"[^>]*>/);
  assert.ok(box,'the checkbox is in the talk key step');
  ...
  assert.equal(html.match(/id="frBackEnter"/g).length,1,'offered once');
  // A key choice beside the talk keys, before the step's Back / Next.
  assert.ok(step.indexOf('id="frTalkCap"')<step.indexOf('id="frBackEnter"'));
  assert.ok(step.indexOf('id="frBackEnter"')<step.indexOf('class="fr-actions"'));
...
test('History offers Off / 24 hours / 7 days and shows what each keeps',()=>{
  ...
  const panel=between('<div class="panel" data-panel="history">','<!-- ABOUT & HELP -->');
...
test('Home counts this launch only, not History restored from earlier sessions',()=>{
  ...
  function home(status){
    ...
    const shown=[];
    vm.runInNewContext(block,{s:status,t:x=>x,fmtCount:String,fmtMinutes:String,renderInsights(){},renderFirstRunTry(){},
...
test('the data note says uninstalling leaves kept History and Remove… deletes it',()=>{
  ...
  // The one free edition: no trial to keep, no other edition to be kept apart from.
  assert.doesNotMatch(html,/trial days are kept|stored separately from other editions/);
  assert.match(html,/<small id="dataNote">Dragging VocalCode to the Bin does not remove these or kept History; Remove… does\.<\/small>/);
  assert.match(between('  function applyPlatformCopy(){','  function setLang('),/"Uninstalling VocalCode does not remove downloaded models, settings or kept History; Remove… does\."/);
});
