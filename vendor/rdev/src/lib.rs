...
// VocalCode patch: the VK<->Key table, so a caller can run its own
// WH_KEYBOARD_LL and still speak this crate's `Key` names (persisted bindings
// are stored as them).
#[cfg(target_os = "windows")]
pub use crate::windows::{code_from_key as windows_code_from_key, key_from_code as windows_key_from_code};
#[cfg(target_os = "windows")]
