...
thread_local! {
    // Event taps are delivered on the run-loop thread that owns the tap. Flags
    // are therefore thread state, not a process-global mutable singleton; two
    // taps on different threads must not race or compare against each other.
    static LAST_FLAGS: Cell<CGEventFlags> = Cell::new(CGEventFlags::CGEventFlagNull);
}
lazy_static! {
...
/// Turn the borrowed event-tap pointer into the owner wrapper expected by the
/// old core-graphics API. Retaining first balances the wrapper's Drop and keeps
/// the callback's +0 reference ownership unchanged.
pub unsafe fn retained_event(event: CGEventRef) -> Option<CGEvent> {
    if event.is_null() {
        return None;
    }
    let retained = CFRetain(event.cast()) as core_graphics::sys::CGEventRef;
    (!retained.is_null()).then(|| CGEvent::from_ptr(retained))
}
...
pub unsafe fn convert(
    ...
    let option_type = match _type {
        ...
        CGEventType::FlagsChanged => {
            ...
            let previous = LAST_FLAGS.with(|last| {
